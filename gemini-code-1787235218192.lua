...
function Library:CreateWindow(options)
    ...
    -- Added UI Scaling Controller
    local GuiScale = Utility:Create("UIScale", {
        Parent = ScreenGui,
        Scale = 1
    })

    -- Automatic Resolution Auto-Scaling
    local camera = Workspace.CurrentCamera
    local userScaleModifier = 1

    local function UpdateScaling()
        if camera then
            local viewportSize = camera.ViewportSize
            -- Base reference resolution: 1920 x 1080
            local baseWidth = 1920
            local baseHeight = 1080
            local scaleX = viewportSize.X / baseWidth
            local scaleY = viewportSize.Y / baseHeight
            
            -- Keep scale proportional and clamp lower/upper bounds
            local autoScale = math.clamp(math.min(scaleX, scaleY), 0.6, 1.25)
            GuiScale.Scale = autoScale * userScaleModifier
        end
    end

    if camera then
        camera:GetPropertyChangedSignal("ViewportSize"):Connect(UpdateScaling)
        UpdateScaling()
    end
    ...
    local Window = {}
    Window.ScaleModifier = userScaleModifier

    function Window:SetUserScale(scalePercent)
        userScaleModifier = scalePercent / 100
        UpdateScaling()
    end
...
local AimbotTab = Window:CreateTab("Aimbot", "107058246184363")

Window:CreateCategory("Config")
local ConfigTab = Window:CreateTab("Settings", "18397244089")
