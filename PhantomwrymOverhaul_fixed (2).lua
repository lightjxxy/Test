...
local Window = Fluent:CreateWindow({
    Title = "HyperionX - Evade Overhaul│Mobile",
...
-- Button Gradients (shared UI colours used throughout the hub)
...
-- UNC Requirements (soft checks - warn only, don't kick so hub still loads)

if not require then
    warn("HyperionX: require() not supported - some features may not work")
else
    print("Supported require()")
end

if not firetouchinterest then
    warn("HyperionX: firetouchinterest() not supported - some features may not work")
else
    print("Supported firetouchinterest()")
end

if not setfpscap then
    warn("HyperionX: setfpscap() not supported - FPS unlock may not work")
else
    setfpscap(500)
...
function DFunctions.InstantReviveNoEmote()
    local nearestPlayer = DFunctions.getNearestDownedPlayer()
    if not nearestPlayer then return end
    for i = 1, 7 do
        ReplicatedStorage.Events.Character.Interact:FireServer("Revive", nil, tostring(nearestPlayer))
